...
from .youtube_normal import YouTubeNormal
...
## Action "Read Audiobook".
#
#  Pepper plays an audiobook video from YouTube on its tablet.
#  This action requires the installation of the NAOqi app PlayYouTube.
class ReadAudiobook(YouTubeNormal):
    ...
    def __init__(self, apar, cpar, session, asr):
        YouTubeNormal.__init__(self, apar, cpar, session, asr)

        self.params = self.loadParameters("audiobooks.json")
...
if __name__ == "__main__":
    ...
    # Run Action
    apar = '"n/a" "n/a"'
    cpar = "1.0 100 1.1 english John romeoAndJuliet&&warAndPeace"

    caressestools.startPepper(session, caressestools.Settings.interactionNode)
    action = ReadAudiobook(apar, cpar, session, "normal")
