...
class Recognizer(AudioSource):
    ...
    @staticmethod
    def setGoogleCredentials(credentials):
        try:
        ...
        with PortableNamedTemporaryFile("w") as f:
            f.write(credentials)
            f.flush()
            Recognizer.api_credentials = GoogleCredentials.from_stream(f.name)

    @staticmethod
    def getGoogleCredentials():
        return Recognizer.google_credentials

    @staticmethod
    def setPreferredPhrases(phrases):
        Recognizer.preferred_phrases = phrases

    @staticmethod
    def getPreferredPhrases():
        return Recognizer.preferred_phrases
    ...
    def recognize_sphinx(self, audio_data, language="en-US", keyword_entries=None, grammar=None, show_all=False):
        ...
        assert keyword_entries is None or all(isinstance(keyword, (type(""), type(""))) and 0 <= sensitivity <= 1 for keyword, sensitivity in keyword_entries), "``keyword_entries`` must be ``None`` or a list of pairs of strings and numbers between 0 and 1"
    ...
    def recognize_google_cloud(self, audio_data, credentials_json=None, language="en-US", preferred_phrases=None, show_all=False):
        ...
        if preferred_phrases is None:
            preferred_phrases = Recognizer.preferred_phrases
        ...
        assert isinstance(language, str), "``language`` must be a string"
        assert preferred_phrases is None or all(isinstance(preferred_phrases, (type(""), type(""))) for preferred_phrases in preferred_phrases), "``preferred_phrases`` must be a list of strings"
        ...
        if credentials_json is None:
            if Recognizer.api_credentials is None:
                api_credentials = GoogleCredentials.get_application_default()
            else:
                api_credentials = Recognizer.api_credentials
        else:
            Recognizer.setGoogleCredentials(credentials_json)
            api_credentials = Recognizer.api_credentials
