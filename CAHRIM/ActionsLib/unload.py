...
## Action "Unload".
#
#  Pepper turns around to let the user remove an object from its optional backpack.
class Unload(Action):
    ...
    def __init__(self, apar, cpar, session, output_handler, asr):
        ...
        self.msg_content = "[(object-at %s):%s]"
    ...
    def run(self):
        ...
        if not self.is_stopped:
            ...
            location = OdomConverter.getCurrentNode()
            self.output_handler.writeSupplyMessage("publish", "D6.2", self.msg_content % (self.object_id, location))
...
if __name__ == "__main__":
    ...
    action = Unload(apar, cpar, session, None, "normal")
    action.run()
