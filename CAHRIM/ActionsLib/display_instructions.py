...
#
#  Pepper shows an instruction video from YouTube on its tablet.
#  This action requires the installation of the NAOqi app PlayYouTube.
class DisplayInstructions(YouTubeTv):
    ...
    def __init__(self, apar, cpar, session, asr):
        ...
        self.subaction = "INSTRUCTIONS"
        self.params = self.loadParameters("instructions_videos.json")
...
if __name__ == "__main__":
    ...
    # Run Action
    apar = '"n/a" "n/a"'
    cpar = "1.0 100 1.1 english John indianClothes&&pudla"

    caressestools.startPepper(session, caressestools.Settings.interactionNode)
    action = DisplayInstructions(apar, cpar, session, "normal")
