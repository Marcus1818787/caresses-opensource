...
# README
# To test the action you need to use a map saved as json file, to be passed as first parameter of the action.
# The map can be drawn by launching the file 'draw.py' in the directory 'ActionsLib\aux_files\go_to\'.
# You can use the map already saved in that directory. To view it, launch 'draw.py' and open it from the 'File' menu.
#
# #=================================================================
#
# w = world (pose in which Pepper wakes up)
# r0 = robot ("initial" pose of Pepper)
# r = robot ("final" pose of Pepper)
# m = map   (pose of the map origin)
# c = charging-station (pose of the charging station)
#
# wf = world-frame
# r0f = robot0-frame
# rf = robot-frame
# mf = map-frame
# cf = charging-station frame
#
# e.g:
#
# m_wf = map origin in world frame
# r_wf = robot pose in world frame
# r_mf = robot pose in map frame
...
## Action "Go To".
#
#  Pepper goes to a specified node of the map.
class GoTo(Action):
    ...
    def end(self):
        ...
        if self.goal_id == "charger":
            ...
            if caressestools.Settings.interactionNode == "":
                ...
                if self.goal_id == caressestools.Settings.interactionNode:
                    msg = "[(:goal(?G1 move-closer-farther true)(?G2 accept-request true))(:temporal (before ?G1 ?G2 [1500 inf]))]"
                else:
                    msg = "[(:goal(?G1 accept-request true))]"
...
if __name__ == "__main__":
    ...
    n = GoTo(apar, cpar, session, None, "normal")
