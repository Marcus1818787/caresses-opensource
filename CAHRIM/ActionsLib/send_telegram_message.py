...
# README
# Each possible recipient should:
# - download Telegram on his phone
# - look for the @myidbot bot and start a "chat" with it
# - type: /getid
# - this will return a numeric id, which corresponds to the recipient Telegram id
# - look for the @Caresses_bot bot and start a "chat" with it
#
# In the setup phase:
# - if testing the whole CARESSES software:
#     - the Telegram id of the user should put in the ontology
# - if testing the action or CAHRIM alone:
#     - the Telegram id of the user should be written in the ./parameters/contacts.json file
#
# When the action is executed and the message is delivered, the recipient will receive a message from the bot.
...
#
# Pepper sends a message through the Telegram app to a person in the contact list.
class SendTelegramMessage(Action):
    ...
    def __init__(self, apar, cpar, session, asr):
        ...
        ## Get only the recipients whose Telegram ID is present
        for id in self.recipient_IDs:
            if not self.recipient_params["IDs"][id]["telegram"].encode('utf-8') == "":
    ...
    def run(self):
        ...
        self.recipient_telegram = self.getAttributeFromID(self.recipient_params, self.recipient_id, "telegram")
        ...
        result = self.sendMessage(self.recipient_telegram, self.message.replace("$USERNAME$", self.username))
    ...
    def sendMessage(self, recipient, message):
        r = requests.post(TELEGRAM_BOT_URL % (recipient, message))
